...
const Assign = () => {
  ...
  useEffect(() => {
    const featchdata = async () => {
      try {
        const response = await axios.get("http://localhost:4306/borrowers");
  ...
  useEffect(() => {
    const featchdata = async () => {
      try {
        const response = await axios.get("http://localhost:4306/lenders");
  ...
  const handleAssigntotable = () => {
    ...
    axios.post("http://localhost:4306/checkforApprovalTABEL", {
      ...
      if (!borrowerExists) {
        ...
        return axios.post("http://localhost:4306/assign", {
          borrowerId: borrowerIdToUpdate,
  ...
  useEffect(() => {
    const featchdata = async () => {
      try {
        const responce = await axios.get("http://localhost:4306/retriveApprovals");
  ...
  const handelSave = (approvalId) => {
    ...
    axios.post(`http://localhost:4306/updatetoAproval/${approvalId}`, updatedData)
      .then(responce => {
